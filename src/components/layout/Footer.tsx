    ...
    <footer className="bg-shefel-red px-4 py-6 mt-auto">
      <div className="max-w-6xl mx-auto text-center font-body text-sm text-shefel-yellow">
        <SocialLinks
          ...
          linkClassName="flex items-center gap-1.5 text-shefel-yellow hover:text-shefel-white transition-colors no-underline font-body font-bold text-sm"
          iconClassName="w-4 h-4"
        />
        <p className="font-bold">&copy; {new Date().getFullYear()} כדורגל שפל. כל הזכויות שמורות.</p>
        <Link
          to="/accessibility"
          className="inline-block mt-2 text-shefel-yellow/70 hover:text-shefel-white transition-colors no-underline font-body text-xs"
        >
          הצהרת נגישות
        </Link>
      </div>
